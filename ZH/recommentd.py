...
def main():
    ...
    # 只爬取1页测试
    success = scraper.scrape_zhihu(pages=1, max_comments=10)
    # comment = scraper.get_comments('1985012426832377113',10)
    if success:
        # 统计结果
        total_rows = len(scraper.all_data)
        rows_with_comments = sum(1 for row in scraper.all_data if row.get('评论内容'))
        print(f"总数据行数: {total_rows}")
        print(f"有评论的行数: {rows_with_comments}")

        scraper.save_to_excel("知乎推荐.xlsx")
    else:
        print("爬取失败")
